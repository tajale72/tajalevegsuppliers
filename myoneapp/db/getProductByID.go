...
func (dbClient *DBClient) GetProductByID(id int) (model.Request, error) {
	...
	if dbClient.DB != nil {
		err = dbClient.DB.QueryRow("SELECT * FROM Bill_Details WHERE id = $1;", id).
			Scan(&bill.ID, &bill.BillNumber, &bill.BillDate, &bill.BillTotalAmount, &bill.SellerName, &bill.SellerPanNum, &bill.CustomerName, &bill.CustomerLocation, &bill.CustomerPhoneNumber, &bill.CustomerPanContainer, &itemsJSON, &bill.TotalDebit, &bill.TotalCredit, &bill.CustomerID)
		if err != nil {
